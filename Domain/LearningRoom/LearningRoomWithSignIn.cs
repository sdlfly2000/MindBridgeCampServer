...
{
    public class LearningRoomWithSignIn : ILearningRoomWithSignIn
    {
        private readonly IRoomAspect _roomAspect;

        public LearningRoomWithSignIn(IRoomAspect roomAspect)
        {
            _roomAspect = roomAspect;
            SignIns = new List<ISignInAspect>();
        }

        public IList<ISignInAspect> SignIns { get; set; }
